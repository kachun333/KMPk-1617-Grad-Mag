...
app.get("/graduates", (req, res) => {
  ...
  admin
    ...
    .then((snapshot) => {
      ...
      console.log(
        "fetching data for ",
        authenticated ? "successfully" : "NOT",
        "AUTHENTICATED user"
      );
      admin
        .firestore()
            ...
            (doc) =>
              new Promise((resolve, reject) => {
                ...
                const bucket = gcs.bucket(
                  "gs://ourpromise.appspot.com/graduates"
                );
                ...
                file
                  ...
                  .catch(() =>
                    reject(`fail to get graduate image url for ${data.name}`)
                  );
              })
          );
          Promise.all(actions)
...
app.get("/magazine/chapIntro/:chapId", (req, res) => {
  ...
  admin
    .firestore()
    .collection("magazine_chap_intro")
    .doc(chapId)
    .get()
    .then((snapshot) => {
      const data = snapshot.data();
                      ...
                      const actions = snapshot.docs.map(
                        (doc) =>
                          new Promise((resolve, reject) => {
                            ...
                            innerData.id = doc.id;
                            const bucket = gcs.bucket(
                              "gs://ourpromise.appspot.com/lecturers"
                            );
                            ...
                            if (!file) {
                              ...
                              file
                                ...
                                .catch(() =>
                                  reject(
                                    `fail to get graduate image url for ${data.name}`
                                  )
                                );
                            }
                          })
